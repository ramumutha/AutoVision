...
| TR-SP-002 | Completed work must not trigger unsafe action | SP-DEMO-004 and suppression policy/tests | PDR-001 | SP-F014 | SP-R002: suppressed opportunity displays â€œDo not actionâ€ and reason in every detail workflow | ADR-004 | Suppression domain state and `ServiceProfitOpportunityDetailComponent` | Suppression domain/repository/UI tests | SP-DEMO-004 | Zero known suppression-semantic regressions in automated tests; production incident KPI RESEARCH REQUIRED |
...
| TR-SP-007 | Reviewed opportunities need accountable internal handling and outcomes | [RS-005 package](../research/RS-005-dealer-follow-up-workflow-validation.md); fieldwork/findings pending | PDR-007 | SP-F016, SP-F016-A | SP-R007 candidate: authenticated user self-claims permitted opportunity, records due state and controlled disposition, and retains auditable history without sending customer communication | ADR-001, ADR-002, ADR-004; new ADR only if boundaries change | None | None | Future: Detected â†’ Reviewed â†’ Accountable follow-up â†’ Outcome/disposition | Workflow fit, handling, disposition, conversion, and recovered-revenue measures RESEARCH REQUIRED |
| TR-SP-008 | Dealers need to know whether available source data supports credible commercial attribution | R1 readiness policy/manifest and synthetic capability evidence; production provider validation pending | PDR-001 | SP-F004 | SP-R008: authenticated manager can see a bounded capability disclosure derived from the latest tenant assessment without exposing raw readiness scores or treating synthetic coverage as production evidence | ADR-001, ADR-002 | `platform/.../serviceprofit/data`; capability read API; frontend manager data-capability disclosure | Repository/read-service/controller integration tests; capability API/component/manager orchestration tests | Manager data-capability disclosure | Capability contracts and UI tests pass; provider-specific production coverage, attribution accuracy, and ROI remain RESEARCH REQUIRED |
| TR-SP-009 | Managers need a compact path from commercial value to focused review and safe action | R1 synthetic scenarios and implemented responsive manager evidence; dealer workflow validation pending | PDR-001, PDR-002, PDR-004 | SP-F003, SP-F007, SP-F010, SP-F011, SP-F012, SP-F013 | SP-R009: manager can move from recoverable potential and KPI lenses to Group By organization, selectively expand groups, inspect evidence, and use desktop/tablet inline or mobile routed detail without treating a page as the full population | ADR-003, ADR-004, ADR-005 | `frontend/.../service-profit`; grouped queue retrieval and manager presentation | Angular manager/grouped-queue suites; responsive, accessibility/Axe, and Service Profit smoke evidence | R1 Service Profit manager walkthrough | Implemented behavior and browser evidence pass; production usability/outcome KPI RESEARCH REQUIRED |
| TR-SP-010 | Authorized dealer data needs a portable and auditable path into Service Profit | RS-006 evidence gap and RS-001 synthetic-only boundary; first-partner discovery pending | ADR-007; G6.1 foundation | SP-F004, SP-F020 | SP-R010: a provider-neutral envelope and versioned canonical file contract define contained lineage, capability-specific readiness, safe missing-data semantics, quarantine, and replay without implementing ingestion | ADR-001, ADR-002, ADR-007 | `contracts/r1-controlled-dealer-file-contract.md`; `docs/architecture/controlled-dealer-data-intake.md` | G6.1 documentation/contract validation; runtime tests none by design | Future controlled FULL-file pilot | Contract/governance evidence only; provider readiness, production ROI, recovered revenue, complete gross profit, and consent RESEARCH REQUIRED |
...
- Search remains deferred: **SEARCH DEFERRED — authoritative server search contract required.** Client-only search over loaded pages would not represent the complete server-paginated dataset.
- The primary R1 manager intentionally hides advanced filters while preserving underlying API/query filtering capability; this is a presentation decision, not platform filter removal.
