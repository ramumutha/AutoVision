...
| Manager summary and queue | Yes | Backend summary/query + Angular/E2E | Currency/count scenarios | Desktop/tablet/mobile; KPI lenses, Group By, collapsed groups | Yes | Prioritized commercial overview | Step 2-3 | Production KPI uplift not established | GREEN |
| Manager organization/sort/detail | Yes | Grouped queue, manager, mobile/detail, responsive/Axe E2E | Grouped synthetic scenarios | Desktop/tablet inline detail; mobile routed detail | Yes | Focused review before action | Steps 3-4 | Search is deferred; primary advanced filters are not exposed | GREEN |
...
| Data readiness/capability | Yes | Policy/loader/manifest plus capability API/UI tests | Coverage/capability manifest | On-demand manager capability disclosure from Recoverable Potential | Yes; underlying synthetic loader remains demo profile gated | Makes supported and partial commercial attribution capability explicit | Manager disclosure / presenter note | Disclosure is implemented, but coverage values are synthetic expectations and provider-specific production readiness is unvalidated | AMBER |
