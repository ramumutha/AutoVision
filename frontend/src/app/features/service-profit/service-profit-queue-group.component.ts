...
@Component({
  ...
  template: `<section class="group" [attr.aria-labelledby]="headingId()"><div class="group-title"><h3 [id]="headingId()">{{ groupLabel() }}</h3><span>{{ group().items.length }}</span></div><ul class="cards">@for (item of group().items; track item.followUpId) { <li><app-service-profit-opportunity-card [item]="item" [ownership]="ownership()" [queryParams]="queryParams()" /></li> }</ul></section>`,
