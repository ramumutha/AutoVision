...
@Component({
  ...
  template: `
    ...
    <section class="workspace" aria-labelledby="queue-heading" [attr.aria-busy]="loading() || refreshing()">
      ...
      @if (loading()) { <p class="state" role="status" aria-live="polite">{{ localization.text('loadingWorkQueue') }}</p> } @else if (error()) { <av-feedback kind="error" [title]="localization.text('unableToLoadWorkQueue')" [message]="localization.text('tryAgain')" /><button class="apply" type="button" (click)="refresh()">{{ localization.text('retry') }}</button> } @else if (!groups().length) { <av-feedback [title]="localization.text('noWorkQueueItems')" [message]="localization.text('noWorkQueueItemsContext')" /> } @else { @for (group of groups(); track group.key) { <app-service-profit-queue-group [group]="group" [groupLabel]="groupLabel(group.key)" [ownership]="ownership()" [queryParams]="queueQueryParams()" /> } @if (page() && page()!.page + 1 < page()!.totalPages) { <button class="load-more" type="button" (click)="loadMore()">{{ localization.text('loadMore') }}</button> } }
    </section>
  `,
  styles: [`
...
export class ServiceProfitWorkQueueComponent {
  ...
  protected readonly queueQueryParams = computed<Params>(() => ({ ownership: this.ownership() === 'ALL' ? undefined : this.ownership(), handlingStatus: this.handlingStatus() || undefined, dueState: this.dueState() || undefined, disposition: this.disposition() || undefined, groupBy: this.groupBy() === 'DUE_STATE' ? undefined : this.groupBy(), sort: this.sort() === 'PRIORITY' ? undefined : this.sort() }));
