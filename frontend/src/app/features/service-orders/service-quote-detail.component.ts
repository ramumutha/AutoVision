...
@Component({
  ...
  imports: [AsyncPipe, DatePipe, DecimalPipe, AvStatusComponent, AvFeedbackComponent, ServiceQuoteAuthorizationRequestComponent, ServiceQuoteAuthorizationDecisionComponent],
    ...
    @if (state$ | async; as state) {
      ...
      @else if (state.detail; as detail) {
        <article class="detail" aria-labelledby="quote-detail-title">
          ...
          @if (decisionRecordedSuccess()) { <av-feedback kind="success" [title]="localization.text('customerAuthorizationDecisionRecorded')" [message]="localization.text('customerAuthorizationDecisionRecordedMessage')" /> }
  ...
  styles: [`
    ...
    .decision-actions { display: flex; flex-wrap: wrap; gap: .5rem; margin-top: 1rem; }.decision-actions button { min-block-size: 2.35rem; padding: .5rem .7rem; border: 1px solid var(--av-color-border); border-radius: var(--av-radius-sm); background: var(--av-color-surface); color: var(--av-color-ink); cursor: pointer; font-weight: 700; }
...
export class ServiceQuoteDetailComponent {
  ...
  protected readonly decisionRecordedSuccess = signal(false);
  private readonly authorizationHistoryContext = signal<AuthorizationHistoryContext | null>(null);
  protected readonly decisionPanel = signal<{ authorizationId: string; action: CustomerAuthorizationDecisionAction } | null>(null);
  ...
  protected openDecision(authorization: CustomerAuthorization, action: CustomerAuthorizationDecisionAction): void { this.decisionRecordedSuccess.set(false); this.decisionPanel.set({ authorizationId: authorization.id, action }); }
  protected closeDecision(): void { this.decisionPanel.set(null); }
  protected decisionRecorded(_authorization: CustomerAuthorization, detail: ServiceQuoteDetail): void { this.decisionPanel.set(null); this.decisionRecordedSuccess.set(true); this.handleLoadedQuote(detail); }
