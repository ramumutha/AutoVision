...
import { fetchServiceEventsForVehicle, fetchVehicleById } from "@/lib/api";
...
export function VehicleDetail() {
  ...
  useEffect(() => {
    async function loadServiceEvents() {
      ...
      try {
        const data = await fetchServiceEventsForVehicle(vehicle.id);
        setServiceEvents(data);
      } catch (loadError) {
        setServiceEventsError(loadError instanceof Error ? loadError.message : "Service events could not be loaded.");
        setServiceEvents([]);
      } finally {
        setLoadingEvents(false);
      }
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <h2 className="text-xl font-semibold text-slate-900">Service Events</h2>
          ...
          {loadingEvents ? (
            ...
            <div className="mt-5 space-y-3">
              {serviceEvents.map((event) => {
                const summary = event.complaint?.structuredSummary?.trim() || event.complaint?.originalText || "No summary available";

                return (
                  <Link
                    key={event.id}
                    href={`/service-intake?vehicleId=${vehicle.id}&eventId=${event.id}`}
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <span className="text-xs font-medium uppercase tracking-wide text-slate-500">{event.source}</span>
                    </div>

                    <dl className="mt-4 grid gap-3 text-sm text-slate-700 sm:grid-cols-2">
                      <div>
                        <dt className="text-xs uppercase tracking-wide text-slate-500">Created</dt>
                        <dd className="mt-1 font-medium">{formatDate(event.createdAt)}</dd>
                      </div>
                      <div>
                        <dt className="text-xs uppercase tracking-wide text-slate-500">Opened</dt>
                        <dd className="mt-1 font-medium">{event.openedAt ? formatDate(event.openedAt) : "Not opened"}</dd>
                      </div>
                      <div>
                        <dt className="text-xs uppercase tracking-wide text-slate-500">Revision</dt>
                        <dd className="mt-1 font-medium">{event.revision}</dd>
                      </div>
                    </dl>

                    <p className="mt-4 text-sm leading-6 text-slate-700">
                      <span className="font-medium text-slate-900">Summary:</span> {summary}
                    </p>
                  </Link>
                );
              })}
      ...
      <div className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <Link
          href={`/service-intake?vehicleId=${vehicle.id}`}
